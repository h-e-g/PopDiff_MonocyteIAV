...
perInf <- filter(keepMeta, COND=="IAV") %>% group_by(ID1, TP) %>% dplyr::count(class) %>% data.frame() %>% dcast(ID1+TP~class)
perInf[is.na(perInf)] <- 0
perInf$per <- rowSums(perInf[,4:5])/rowSums(perInf[,3:5])*100
perInf$perHigh <- perInf[,4]/rowSums(perInf[,4:5])*100

tp4 = filter(perInf, TP==4)
...
two <- t0 %>% group_by(ID1) %>% dplyr::count(sxCD16) %>% data.frame() %>% dcast(ID1~sxCD16)
two$ratio <- two$`FALSE`/two$`TRUE`
two$per <- two[,3]/rowSums(two[,2:3])*100

#look for correlation to flu phenotypes
compDF <- dcast(comp[,c(1, 2, 5)], ID1 ~ T0subset)
compDF$perInf <- tp4[match(compDF$ID1, tp4$ID1), 'per']

cor.test(compDF$classical, compDF$perInf)
cor.test(compDF$nonclassical, compDF$perInf)
cor.test(compDF$intermediate, compDF$perInf) #intermediate monocytes again correlate with flu levels

two$perInf <- tp4[match(two$ID1, tp4$ID1), 'per']
cor.test(two$ratio, two$perInf)
cor.test(two$per, two$perInf)
...
phenotype <- tp4[match(levels(t0$ID1), tp4$ID1),'per']
...
table(classical2$fluCor_pval < 0.01) #126 -> 113/3121
table(intermediate2$fluCor_pval < 0.01) #6 -> 7/188
table(nonclassical2$fluCor_pval < 0.01) #18 -> 21/253
...
write.table(t0.master, file=paste0(PopDiff, "results/T0_three_subsets_stats_4589genes_wInterindiviudalVariability_perInfected4h.txt"), row.names=F, col.names=T, sep="\t", eol="\n", quote=F, qmethod=c("double"))
...
interest <- unique(c(nonclassicList, classicList, intermediateList)) #135 genes sig diff between donors & nominally correlates with flu in at least one subset -> 118
...
write.table(goRES[,-9], paste0(PopDiff, "results/T0_fluCor_subsetspecific_GO.enrichments_4589backgroundgenes_ONELIST_perInfected4h.txt"), row.names=F, col.names=T, sep="\t", eol="\n", quote=F, qmethod=c("double"))

interestGA <- filter(GA, stable %in% interest)
keep <- interestGA[grepl("GO:0051607|GO:0034340", interestGA$GO),'Symbol']
...
write.table(table1, file=paste0(PopDiff, "results/table1_27genes_raw.txt"), row.names=F, col.names=T, sep="\t", eol="\n", quote=F, qmethod=c("double"))
write.table(interest, file=paste0(PopDiff, "results/118gene_candidates_ensemblID.txt"), row.names=F, col.names=F, sep="\n", eol="\n", quote=F, qmethod=c("double"))
